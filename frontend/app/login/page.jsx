...
export default function LoginPage() {
  ...
  const completeGoogleLogin = async ({ credential }) => {
    ...
    try {
      ...
      setLoading(false);
      setGoogleLoading(false);
      if (googleTimer.current) window.clearTimeout(googleTimer.current);
    }
  };
  ...
  const google = () => {
    ...
    window.google.accounts.id.prompt((notification) => {
      if (notification.isNotDisplayed() || notification.isSkippedMoment() || notification.isDismissedMoment()) {
        setGoogleLoading(false);
        setError("Google sign-in was cancelled or could not open. Check that localhost:3000 is allowed in Google Cloud Console.");
      }
    });
    googleTimer.current = window.setTimeout(() => {
      setGoogleLoading(false);
  ...
  const initializeGoogle = () => {
    ...
    window.google.accounts.id.initialize({ client_id: googleClientId, callback: completeGoogleLogin, use_fedcm_for_prompt: false, auto_select: false, cancel_on_tap_outside: true });
    setGoogleReady(true);
  };
